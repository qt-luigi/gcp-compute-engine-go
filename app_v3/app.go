...
// path 変数は、インストールパスです。
// ローカル実行の際は""に変更してください。
const installPath = "/opt/dengonban/v3"
...
var (
	bucketName  = fmt.Sprintf("%s-imagestore", projectid)
	storagePath = fmt.Sprintf("https://storage.cloud.google.com/%s", bucketName)
)
...
// IsImage 関数は、対応している画像ファイルの拡張子か判定します。
// 対応している画像ファイルの拡張子：jpg, jpeg, png, gif
...
func NewMessageForm(r *http.Request) MessageForm {
	...
	form.InputPhoto = NewUploadField(r, "input_photo", "画像添付(jpg, jpeg, png, gif)", []Validator{Upload{Func: IsImage}})
	return form
}
...
func post(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" && form.Validate() {
		...
		ferr := form.InputPhoto.FileError
		...
		if ferr == nil {
			...
			if err := db.Add(Message{Timestamp: timestamp, Name: name, Message: message, Filename: filename}); err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
		} else {
